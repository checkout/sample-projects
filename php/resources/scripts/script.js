...
// When the tokenization is completed retrieve the card token and send to the server to complete a payment
Frames.addEventHandler(Frames.Events.CARD_TOKENIZED, (event) => {
    // Send the token to the server to complete the payment
    http(
        {
            method: 'POST',
            route: '/controller/PaymentController.php',
            body: {
        ...
        (response) => {
            ...
            if (response.status >= '300') {
                ...
                response.json().then((data) => (displayApiResult(data)));
            }
        }
    );
});
...
function displayApiResult(response) {
    ...
    el.innerHTML =
        `Payment outcome: <span class="payment-response">${response.response_summary}</span>` +
